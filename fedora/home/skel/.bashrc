...
export HISTTIMEFORMAT="%Y-%m-%d %H:%M:%S " # trailing space makes history log look nicer
...
export PROJECT_HOME="/usr/src/projects"
...
if [ -f $USR_BIN/virtualenvwrapper.sh ]; then
    ...
    if [ -f /usr/bin/virtualenvwrapper.sh ]; then
        ...
        echo "Can't find a virtualenvwrapper script to run."
    fi  
fi
...
# Separate Alias Definition File Imported Here
...
MOREUSERPATHS=("." "${HOME}/bin" "${HOME}/bin/python-scripts" "/sbin")
...
# somehow pip install scipy still fails, despite these env variables
if [ -f $HOME/src/labpack-*/liblapack.a ]; then
    export BLAS=~/src/BLAS/libfblas.a
    export LAPACK=$HOME/src/lapack-*/liblapack.a
fi

# default folders for sharing python projects
# * /usr/local/share

# share virtualenvs among all users
if [ -d ${HOME}/.virtualenvs ]; then
   :
else
   if [ -d ${WORKON_HOME} ]; then
      ln -s ${WORKON_HOME} ${HOME}/.virtualenvs
   else
      mkdir -p ${HOME}/.virtuanenvs
   fi
fi

# share virtualenv wrapper projects (python source code) among all users
if [ -d ${HOME}/src ]; then
    :
else
   if [ -d ${PROJECT_HOME} ]; then
      ln -s ${PROJECT_HOME} ${HOME}/src
   else
      mkdir -p ${HOME}/src 
   fi
fi
